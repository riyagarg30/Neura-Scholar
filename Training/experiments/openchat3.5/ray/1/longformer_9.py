    ...
    def train_func():
        ...
        def loss_logger(loss_value, epoch, steps):
            if loss_value is not None:
                step_val = steps if steps is not None else epoch * len(dataloader)
                mlflow.log_metric("train_loss", float(loss_value), step=step_val)
                train.report({"train_loss": float(loss_value), "step": step_val})
                print(f"Train loss: {loss_value:.4f} at step {step_val}")
